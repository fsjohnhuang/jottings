...
**基本权限类型**<br/>
1. r(读):对于普通文件就是查看的权限;对于目录就是进入目录内部的权限,配置x权限则可以查看目录的详细信息<br/>
2. w(写):对普通文件就是修改、删除的权限，需要配置r权限一起使用;对于目录就是添加、删除内部文件和删除目录的权限，需要配置x权限一起使用。<br/>
3. x(执行):对于可执行文件就是可执行的权限。对于shell脚本文件则需要配置r权限才可执行；对于目录则是可将该目录设置为工作目录<br/>
**特殊权限??**<br/>
1. SUID:仅对二进制可执行文件，拥有SUID权限的用户在执行程序时，程序可获得程序所有者的权限支援。<br/>
2. SGID:对二进制可执行文件，拥有SGID权限的用户在执行程序时，程序可获得程序所属组的权限支援;目录的所属组不是所有者的群组????<br/>
3. SBIT:用户可以在该目录下删除、移动自己创建的目录和文件，需要配置w，x权限。<br/>
数字表示法：<br/>
SUID=4, SGID=2, SBIT=1<br/>
字符表示法：<br/>
